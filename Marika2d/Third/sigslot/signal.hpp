...
namespace sigslot
{
    namespace
    {
        bool is_globe_signal_id_repeat(std::string_view id)
        {
            static std::set<std::string> checker;
            auto ret = checker.find(id.data());
            if (ret != checker.end())
            {
                return true;
            }
    ...
    {
    public:
        static inline signal<T...>& get_signal(std::string_view id)
        {
            std::lock_guard<std::mutex> lock(mutex);
            auto ret = signals.try_emplace(id.data(), signal<T...>());
            if (ret.second && is_globe_signal_id_repeat(id))
            {
                throw std::runtime_error(std::format("signal [{}] has already existed ! did you try to get tow signals but its have different params pkg with same id ? ", id.data()).c_str());
            }
            return ret.first->second;
        }
    private:
        static inline std::map<std::string, signal<T...>> signals;
