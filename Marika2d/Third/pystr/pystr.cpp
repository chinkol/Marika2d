...
#define ADJUST_INDICES(start, end, len)         \
    ...
    {
        if (pystr::startswith(str, prefix))
        {
...
namespace path
{
                       ...
                       const std::string & p)
    {
        if (p.size() >= 2 && p[1] == ':')
        {
            ...
            drivespec = pystr::slice(path, 0, 2);
            pathspec = pystr::slice(path, 2);
        }
        else
        {
    ...
    {
        return pystr::startswith(s, forward_slash);
    }
        ...
        for(unsigned int i=1; i<paths.size(); ++i)
        {
            ...
            else if(isabs_nt(b))
            {
                ...
                // Else path has a drive letter, and b doesn't but is absolute.
                        ...
                        ((path.size()==3) && !pystr::endswith(path, forward_slash) && !pystr::endswith(path, double_back_slash)))
                {
                    b_nts = true;
                }
            }
            ...
            else
            {
                ...
                // assert len(path) > 0
                if( pystr::endswith(path, forward_slash) || pystr::endswith(path, double_back_slash))
                {
                    if(pystr::startswith(b,forward_slash) || pystr::startswith(b,double_back_slash))
                    {
                        path += pystr::slice(b, 1);
                    }
                    else
                    {
                        path += b;
                ...
                else if(pystr::endswith(path, colon))
                {
                    path += b;
                }
                else if(!b.empty())
    ...
    std::string join_posix(const std::vector< std::string > & paths)
    {
        ...
        {
            std::string b = paths[i];
            if(pystr::startswith(b, forward_slash))
            {
                path = b;
            }
            else if(path.empty() || pystr::endswith(path, forward_slash))
            {
                path += b;
            }
            else
    ...
    void split_nt(std::string & head, std::string & tail, const std::string & path)
    {
        ...
        head = pystr::slice(p,0,i);
        tail = pystr::slice(p,i); // now tail has no slashes
        
        // remove trailing slashes from head, unless it's all slashes
        std::string head2 = head;
        while(!head2.empty() && ((pystr::slice(head2,-1) == forward_slash) ||
                                 (pystr::slice(head2,-1) == double_back_slash)))
        {
            head2 = pystr::slice(head2,0,-1);
        }
    ...
    {
        int i = pystr::rfind(p, forward_slash) + 1;
        
        head = pystr::slice(p,0,i);
        tail = pystr::slice(p,i);
        
        if(!head.empty() && (head != pystr::mul(forward_slash, (int) head.size())))
        {
            head = pystr::rstrip(head, forward_slash);
        }
    }
    ...
    {
        std::string path = p;
        path = pystr::replace(path, forward_slash,double_back_slash);
        ...
        if(prefix.empty())
        {
            ...
            while(pystr::slice(path,0,1) == double_back_slash)
            {
                prefix = prefix + double_back_slash;
                path = pystr::slice(path,1);
            }
        }
        else
        {
            // We have a drive letter - collapse initial backslashes
            if(pystr::startswith(path, double_back_slash))
            {
                prefix = prefix + double_back_slash;
                path = pystr::lstrip(path, double_back_slash);
            }
        }
        
        std::vector<std::string> comps;
        pystr::split(path, comps, double_back_slash);
        ...
        while(i<(int)comps.size())
        {
            ...
            else if(comps[i] == double_dot)
            {
                ...
                else if(i == 0 && pystr::endswith(prefix, double_back_slash))
                {
                    comps.erase(comps.begin()+i);
                }
                else
        ...
        return prefix + pystr::join(double_back_slash, comps);
    }
    ...
    std::string normpath_posix(const std::string & p)
    {
        ...
        int initial_slashes = pystr::startswith(path, forward_slash) ? 1 : 0;
        
        // POSIX allows one or two initial slashes, but treats three or more
        // as single slash.
        
        if (initial_slashes && pystr::startswith(path, double_forward_slash)
            && !pystr::startswith(path, triple_forward_slash))
            initial_slashes = 2;
        
        std::vector<std::string> comps, new_comps;
        pystr::split(path, comps, forward_slash);
        ...
        path = pystr::join(forward_slash, new_comps);
        
        if (initial_slashes > 0)
            path = pystr::mul(forward_slash, initial_slashes) + path;
    ...
    {
        int sepIndex = pystr::rfind(p, sep);
        if(!altsep.empty())
        {
            int altsepIndex = pystr::rfind(p, altsep);
            sepIndex = std::max(sepIndex, altsepIndex);
        }

        int dotIndex = pystr::rfind(p, extsep);
        if(dotIndex > sepIndex)
        {
            ...
            {
                if(pystr::slice(p,filenameIndex) != extsep)
                {
                    root = pystr::slice(p, 0, dotIndex);
                    ext = pystr::slice(p, dotIndex);
                    return;
                }
